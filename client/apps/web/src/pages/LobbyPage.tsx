          ...
          <div className="space-y-1 text-left">
            ...
            {room.players.map((p) => (
              <div key={p.userId} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
                <span>{p.name}{p.userId === room.host ? " 👑" : ""}</span>
...
function CreateOrJoin({
  ...
  onCreate: (o: { questionCount: number; timePerQ: number }) => void;
  ...
  return (
    <div className="mx-auto max-w-md space-y-4 p-4">
      ...
      <Card className="space-y-3">
        <p className="text-sm text-slate-500">Soha: Ingliz tili (noto'g'ri fe'llar) · classic</p>
        ...
        <Button className="w-full" onClick={() => onCreate({ questionCount: count, timePerQ: time })}>
          Xona yaratish
        </Button>
      </Card>
